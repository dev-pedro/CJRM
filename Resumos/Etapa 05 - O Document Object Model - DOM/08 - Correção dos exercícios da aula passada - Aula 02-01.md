...
```javascript
const showChallengeName = (number) =>
...
*/
const getCatInfo = () => {
  const catName = "Marcos";
  let age = 3;
  const color = "Cinza";
  return {catName, age, color}
};

const { catName, age, color } = getCatInfo()

console.log(`${catName} é um gato ${color} de ${age} anos.`);
...
randomNumbers.reverse(randomNumbers);
...
const removeFirstItem = crazyArray.shift();
...
const findDogZequinha = dogs.find((dog) => dog.name === 'Zequinha')
console.log(findDogZequinha);
...
*/
const mainTitleQuery = document.querySelector('.main-title')
...
*/
const secundaryTitle = document.querySelectorAll('.secondary-title')
console.log(secundaryTitle)
```
