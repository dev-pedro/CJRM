...
if (houraAtual >= 6 && houraAtual <= 11) {
  console.log("Bom dia");
} else if (houraAtual >= 12 && houraAtual <= 17) {
...
*/
const myAge = 44;
let ingerssoCinema = null;
const isChild = 7;
const isOlder = 65;
if (myAge <= isChild || myAge >= isOlder) {
  ingerssoCinema = "Para você, a entrada é grátis!";
} else {
  ingerssoCinema = "A entrada é R$ 30,00.";
}
console.log(ingerssoCinema);
/*
  04
...
for (let i = 0; i < numbers.length; i++) {
  const num = numbers[i];
  const isNumberBetween11and90 = num >= 11 && num <= 90;
...
for (let i = 0; i < crazyArray.length; i++) {
  ...
  const isItemNumber = typeOfItem === "number";
...
console.log(
  `O crazyArray tem ${numBoolean} booleans, ${numNumbers} números e ${numStrings} strings.`
);
/*
  06
...
const oddNumber = oddNumberArray.join(", ").replace(", 3", " e 3");
const evenNumber = evenNumberArray.join(", ").replace(", 5", " e 5");

console.log(`Numeros ímpares: ${oddNumber}. Números pares: ${evenNumber}.`);
```

A correção desses exercícios foi muito legal pois comecei a ter uma visão melhor sobre refatoração de código na pratica.
