...
# Correção dos exercícios da aula 05 da etapa 11 - Aula 06-02 e Aula 06-02
...
const logGitHubUser = async userName => {
  ...
  console.log(user, objKeys)
}
logGitHubUser('dev-pedro')
...
const getDivisibleBy2Or3 = numbers =>
  numbers.filter(value => value % 2 === 0 || value % 3 === 0)

console.log(numbers, getDivisibleBy2Or3(numbers))
...
const silabaNome = ['Pe', 'dro']

// const pNome = silabaNome.join('P')
const getNameInPLanguage = name =>
  name.reduce((acc, silaba) => `${acc}P${silaba}`, '')
console.log(getNameInPLanguage(silabaNome))
...
const filterItem = (item, index, array) => {
  const isValueTrue = callback(item, index, array)
  if (isValueTrue) {
    newArray.push(item)
  }
}
