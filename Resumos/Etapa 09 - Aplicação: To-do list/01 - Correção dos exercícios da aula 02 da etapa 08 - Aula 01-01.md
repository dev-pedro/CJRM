...
const namesInAlphabecticalOrder = getArrayCopy(names).sort()
console.log(names, namesInAlphabecticalOrder)
...
console.log(charactersOrderById, characters)
/*
  03
...
console.log(numbersInAscendingOrder, numbers)
...
const peopleInReverseAlphabecticalOrder = getArrayCopy(people).sort().reverse()
console.log(peopleInReverseAlphabecticalOrder, people)
/*
  06
...
const frase = ingredients
  .map(item => (/a$/.test(item) ? `${item} cozida` : `${item} cozido`)) //utilizando regex
  //.map(item => (item[item.length-1] === 'a' ? `${item} cozida` : `${item} cozido`))//utilizando comparação direta
  .join(', ')

console.log(frase, ingredients)
/*
  07
...
console.log(dogsInHumanAge, pets)
...
movieNames = topBrazilMovies.reduce(
