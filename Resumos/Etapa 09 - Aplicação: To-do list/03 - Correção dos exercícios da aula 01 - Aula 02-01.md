...
people[0].firstName = 'Pedro'

console.log(peopleOrderedByScore, people)
/*
  03
...
const animaisThreeWordMinimal = animals.filter(({length}) => length === 3)
...
const animalsNumbersWords = animals.map(({length}) => length)
...
const sumNumbersOdd = numbers
  .filter(number => number % 2 === 1) //verifica se o número é impar
