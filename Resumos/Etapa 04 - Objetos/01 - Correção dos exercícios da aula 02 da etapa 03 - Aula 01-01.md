...
console.log(convertToString("Pedro"));
...
*/
const lengthString = (text) => text.length;
console.log(lengthString("Pedro"));
...
*/
const getIndex = (index, string) => string.indexOf(index);
console.log(getIndex("P", "Pedro"));
...
const user = "Pedro";
const arrayNames = ["Rose", "Pedro", "Júlia", "Alice", "Sara"];
console.log(isItemIncluded(user, arrayNames));
...
*/
const concatArrays = (firstArray, secoundArray) =>
  firstArray.concat(secoundArray);
const array1 = [1, 2];
const array2 = [3, 4, 5];
...
const arrayNumbers = concatArrays(array1, array2);
console.log(removeLastItem(arrayNumbers));
...
const nullItem = null;
console.log(valueIsNull(nullItem));
...
const showName = () => console.log("Pedro Henrique");
invokeCallback(showName);

showChallengeNumber(10);
/*
  10
...
const showNumbersInfo = (number, index, array) => {
  console.log(
    `O ${index + 1}º item do array [${array.join(", ")}] é ${number}.`
  );
};
numbers.forEach(showNumbersInfo);

showChallengeNumber(12);
/*
  12
...
const review = [
  "Eu sempre adorei o filme e quando descobri que tinha o livro também fiquei doido. Demorei um pouco mas acabei comprando e finalmente li.",
...
const infoLikesPeople = (names = []) => {
  const firstName = names[0]
  const secondName = names[1]
  const thirdName = names[2]
  const curtiram = "curtiram isso";
  const numberPeople = names.length;
  const peopleMinusTwo = names.length - 2
  ...
  switch (numberPeople) {
    ...
    case 2:
      return `${firstName} e ${secondName} ${curtiram}`;
    case 3:
      return `${firstName}, ${secondName} e ${thirdName}${curtiram}`;
    default:
      return `${firstName}, ${secondName} e mais ${peopleMinusTwo} pessoas ${curtiram}`;
  }
};

console.log(infoLikesPeople(arrayNames));
```
